...
String router(String subroute){
  return BKT_SERVER + (String)":" + API_PORT + subroute;
}
...
void generateSensorId() {
  char espIdAux[12];
  snprintf(espIdAux, 12, "%X%X", ESP.getEfuseMac());
  sensorId = espIdAux;
